...
def _run_phase(
    ...
    *,
    ...
    for round_id in range(max_rounds):
        ...
        backfilled = _propagate_and_backfill(bound, index, tightened, edge_budget=edge_budget, mark=round_id)
    ...
    *,
    ...
    with torch.no_grad():
        ...
        if enable_refinement:
            delete_stats = _run_phase(bound, new_index, allow_raise_lower=False, edge_budget=edge_budget, max_rounds=max_rounds)
            insert_stats = _run_phase(bound, new_index, allow_raise_lower=True, edge_budget=edge_budget, max_rounds=max_rounds)
        else:
            delete_stats = PhaseStats()
            insert_stats = PhaseStats()
        tau_new, cone_pack, used_fallback = _solve_cone_exact(new_index, bound, edge_budget=edge_budget)
