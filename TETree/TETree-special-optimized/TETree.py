...
def equi_tree_construction(row_ptr: torch.Tensor, columns: torch.Tensor, rows: torch.Tensor, truss_result: torch.Tensor):
    ...
    # 返回超节点数据
    return valid_id.to(torch.int32), edge_to_node.to(torch.int32), src_node.to(torch.int32), des_node.to(torch.int32), max_node_id
...
def run_with_truss(filename: str):
    torch.cuda.empty_cache()
